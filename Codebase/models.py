...
class TimeSeriesModelEvaluator:
    ...
    def plot_roc_auc(self, y_tests: np.ndarray, y_preds: np.ndarray, metrics: dict) -> None:
        # Binarize the output
    ...
    def evaluate_no_cv(self, X, y):
        ...
        # if len(classes) > 1:
        #     metrics['roc_auc_score'] = roc_auc_score(y_test, y_pred, average='macro', multi_class='ovo')

        # Plot confusion matrix and ROC AUC
        self.plot_confusion_matrix(metrics['confusion_matrix'])
        # self.plot_roc_auc(y_test, y_pred, metrics)
    ...
    def evaluate(self,
        ...
        if self.hyperparams is not None:
            self.model = GridSearchCV(self.model, self.hyperparams, cv=cv)
